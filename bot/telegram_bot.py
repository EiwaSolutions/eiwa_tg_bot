...
CONNECTION_STRING = "postgresql+psycopg2://postgres:C0c2TKwpSVXH@ep-old-lake-52316654.eu-central-1.aws.neon.tech/mamaspace"
...
class PGVectorSingleton:
    ...
    @staticmethod
    def get_instance():
        if PGVectorSingleton._instance is None:
            PGVectorSingleton._instance = PGVector(
                collection_name=COLLECTION_NAME,
                connection_string=CONNECTION_STRING,
                distance_strategy=DistanceStrategy.COSINE,
                embedding_function=EMBEDDINGS,
            )
        return PGVectorSingleton._instance
...
class ChatGPTTelegramBot:
    ...
    def __init__(self, config: dict, openai: OpenAIHelper):
        ...
        self.store = PGVectorSingleton.get_instance()
        self.commands = [
            # BotCommand(
            #     command="help",
    ...
    async def help(self, update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # )
        help_text = "Ответим на ваши вопросы по лекарствам во время гв и не только. \n\n"
    ...
    async def resend(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await self.handle_query(update=update, context=context)
    ...
    async def handle_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        Handles a query to retrieve and process a document using the provided code snippet
        """
        if not await self.check_allowed_and_within_budget(update, context):
            return

        query_text = message_text(update.message)
        if query_text == "":
            await update.effective_message.reply_text(
                message_thread_id=get_thread_id(update),
                text=localized_text("query_no_text", self.config["bot_language"]),
            )
            return

        logging.info(
            f"New query request received from user {update.message.from_user.name} "
            f"(id: {update.message.from_user.id})"
        )

        async def _handle():
            try:
                # Replace synchronous code with asynchronous code where possible
                # For simplicity, leaving the synchronous code as-is for now

                for _ in range(3):
                    try:
                        logging.info("--> *** Searching by similarity ***")
                        docs = await self.store.asimilarity_search(query_text, k=3)
                ...
                source = most_relevant.metadata["source"]
                path = f"knowledge_base/{source}"

                async with aiofiles.open(path, mode="r") as file:
                    text = await file.read()

                template = (
                    "Ты обрабатываешь текст полученный из базы данных учёных, которые в результате "
                    "тщательной научной работы смогли составить справочник по лекарствам для кормящих мам. "
                    "База называется e-lactancia. Ты просто читаешь текст из базы данных и отвечаешь на вопрос. "
                    "Сообщаешь о вероятных рисках, если о таковых в тексте есть информация. "
                    "Текст из базы данных: \n{kb_text} \n"
                    "Помни, ничего не придумывай! Перед предоставлением ответа, проверь запрос пользователя. "
                    "Не говори про лекарства, которые не были упомянуты пользователем. "
                    "Говори только по тексту. Если ответ не найден, скажи что в базе данных информация не найдена!"
                )
                human_template = "Вопрос по тексту из базы данных: {text}"
                ...
                prompt = chat_prompt.format_messages(kb_text=text, text=query_text)
                ...
                async for token in llm.astream(prompt):
                    ...
                    # Check if buffer size has reached a threshold for sending
                    # Here, we assume a threshold of 100 characters for simplicity
                    if len(buffer) >= 4096:
                ...
                await update.effective_message.reply_text(
                    message_thread_id=get_thread_id(update),
                    reply_to_message_id=get_reply_to_message_id(self.config, update),
                    text=f"{localized_text('query_fail', self.config['bot_language'])}: {str(e)}",
                )

        await wrap_with_indicator(update, context, _handle, constants.ChatAction.TYPING)
